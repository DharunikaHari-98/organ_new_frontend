...
import { Typography, Paper } from '@mui/material'; // <--- CORRECTED THIS LINE
...
const getEventVisuals = (eventType) => {
    switch (eventType) {
        case 'ALLOCATION_CREATED':
            return { icon: <PlaylistAddCheckIcon />, color: 'primary' };
        case 'IN_TRANSIT':
            return { icon: <LocalShippingIcon />, color: 'info' };
        case 'COMPLETED':
            return { icon: <CheckCircleIcon />, color: 'success' };
        case 'CANCELLED':
            return { icon: <HighlightOffIcon />, color: 'error' };
        default:
        ...
        return (
            <Paper sx={{ p: 4, textAlign: 'center' }}>
                ...
                <Typography variant="h6" color="text.secondary">
                    No events have been logged for this allocation yet.
                </Typography>
            </Paper>
        );
    }
