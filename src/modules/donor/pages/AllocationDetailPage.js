...
import { getAllocationEvents } from '../services/donorApi';
...
const AllocationDetailPage = () => {
    ...
    const { data: events, isLoading, isError, error } = useQuery({
        queryKey: ['myAllocationEvents', id],
    ...
    return (
        <Paper sx={{ p: 3, boxShadow: 3, borderRadius: 2 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                ...
                <Button variant="outlined" startIcon={<ArrowBackIcon/>} onClick={() => navigate('/donor/allocations')}>Back to List</Button>
            </Box>
