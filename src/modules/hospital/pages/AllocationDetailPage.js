...
import { getAllocationEvents } from '../services/hospitalApi';
...
const AllocationDetailPage = () => {
    ...
    const { data: events, isLoading, isError, error } = useQuery({ queryKey: ['allocationEvents', id], queryFn: () => getAllocationEvents(id) });
    return (
        <Paper sx={{ p: 3, boxShadow: 3, borderRadius: 2 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <Typography variant="h5" fontWeight="600">Allocation Timeline</Typography>
                <Button variant="outlined" startIcon={<ArrowBackIcon/>} onClick={() => navigate('/hospital/allocations')}>Back to List</Button>
            </Box>
