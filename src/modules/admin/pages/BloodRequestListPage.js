...
import { getBloodRequests } from '../services/adminApi';
...
const BloodRequestListPage = () => {
    ...
    const { data, isLoading, isError, error } = useQuery({
        queryKey: ['bloodRequests', paginationModel],
        queryFn: () => getBloodRequests(paginationModel),
    });
    ...
    const columns = [
        ...
        { field: 'quantityUnits', headerName: 'Units', width: 100 },
        ...
        },
        {
            ...
            renderCell: (params) => (
                <Button
                  ...
                  onClick={() => navigate(`/admin/blood-requests/${params.id}`)}
                >
                    View
                </Button>
            ),
        ...
        <Paper sx={{ p: 3, boxShadow: 3, borderRadius: 2 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                ...
                <Button
                  variant="contained"
                  startIcon={<AddIcon/>}
                  onClick={() => navigate('/admin/blood-requests/new')}
                >
                    New Request
                </Button>
            </Box>
