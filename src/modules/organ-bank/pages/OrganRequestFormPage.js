...
import { createOrganRequest } from '../services/organBankApi';
...
const OrganRequestFormPage = () => {
    ...
    const [formData, setFormData] = useState({ hospitalName: '', hospitalContact: '', organType: '', bloodGroup: '', urgency: 'MEDIUM', notes: '', hospitalCity: '', hospitalState: '' });
    const mutation = useMutation({ mutationFn: createOrganRequest, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['organBankOrganRequests'] }); navigate('/organ-bank/organ-requests'); } });
