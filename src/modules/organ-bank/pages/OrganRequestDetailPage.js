...
import { getOrganRequestById, findOrganMatches, getOrganRequestCandidates } from '../services/organBankApi';
...
const OrganRequestDetailPage = () => {
    ...
    const { data: request, isLoading, isError, error } = useQuery({ queryKey: ['organBankOrganRequest', id], queryFn: () => getOrganRequestById(id) });
    ...
    const findMatchesMutation = useMutation({ mutationFn: () => findOrganMatches(id), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['organBankOrganRequest', id] }); queryClient.invalidateQueries({ queryKey: ['organRequestCandidates', id] }); }, });
        ...
        <>
            <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={8}>
                    <Paper sx={{ p: 2, borderRadius: 2, boxShadow: 3 }}><Typography variant="h6" fontWeight={600} gutterBottom>Match Candidates</Typography><MatchCandidatesTable candidates={candidates || []} isLoading={candidatesLoading} requestId={id}/></Paper>
                </Grid>
            </Grid>
