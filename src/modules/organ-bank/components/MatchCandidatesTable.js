...
import { acceptMatch, declineMatch } from '../services/organBankApi';
...
const MatchCandidatesTable = ({ candidates, requestId }) => {
    ...
    const updateMatchStatus = useMutation({
        ...
        onSuccess: (data, vars) => {
            queryClient.invalidateQueries({ queryKey: ['organRequestCandidates', requestId] });
            if (vars.action === 'accept') {
                setSelectedMatchId(data.id ?? vars.matchId);
                setModalOpen(true);
            }
        },
    });
    ...
    const columns = [
        ...
        { field: 'status', headerName: 'Status', flex: 1 },
        {
            ...
            sortable: false,
            renderCell: (params) => {
                const isActionable = params.row.status === 'PENDING_DONOR_CONFIRM';
