...
import { createAllocation } from '../services/organBankApi';
...
const AllocationCreateModal = ({ open, onClose, matchId }) => {
    ...
    const [formData, setFormData] = useState({ hospitalLocation: '', transportMode: 'AMBULANCE', scheduledAt: '' });
    const mutation = useMutation({
        mutationFn: createAllocation,
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['organBankAllocations'] });
            onClose();
            navigate(`/organ-bank/allocations/${data.id}`);
        },
    });
        ...
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
            ...
            <DialogContent>
                <Grid container spacing={2} sx={{ mt: 1 }}>
                    <Grid item xs={12}><TextField name="hospitalLocation" label="Current Location of Organ" onChange={handleChange} required fullWidth /></Grid>
